...
def Song_str(s: Song) -> str:
    '''Takes in a song and return a string in a readable format'''
    print('      ', s.track, s.title, s.length, s.play_count, sep='---')
    return
##Song_str(Song(10,'The Drop', 179,1))

def Album_str(a: Album) -> str:
    '''Takes in an Album and returns a string containing that albums info'''
...
def Songdisplay_str(s: Songdisplay) -> str:
    '''Takes in a Songdisplay and returns a sring containg that information in
    a easily readable format'''
    return('{} {} {} {} {} {}'.format(s.artist, s.a_title, str(s.year), str(s.track), s.s_title, str(s.length)))
    
x = Album_to_Songdisplays(MUSIC[0])
for item in x:
          Songdisplay_str(item)

          
def song_len(a: Album) -> int:
    return len(a.songs)
##print(song_len(MUSIC[0]))

MUSIC.sort(key=song_len, reverse=True)
##Album_str(MUSIC[0])

##collection_sort(MUSIC, song_len)
##Songdisplay_str(MUSIC)

def unplayed_songs(L: 'list of Album') -> list:
    '''Takes in a list of albums and returns a list of songdisplay, one for each
    song that has never played'''
    result = []
    for a in L:
        a_display = Album_to_Songdisplays(a)
        for s in a_display:
            if s.play_count == 0:
                result.append(s)
...
def length_from_songdisplay(s: Songdisplay) -> int:
    '''Takes in a songdisplay and returns the length of the song'''
    return s.length

##for item in x:
##    print(length_from_songdisplay(item))

def Song_listening_time(s: Song) -> int:
    '''Takes in a single song and returns the total seconds of the song'''
    return s.length * s.play_count

##print(Song_listening_time(Song(1, "Darkness", 411, 5)))
...
def Album_listening_time(a: Album) -> int:
    ...
    return total
##print(Album_listening_time(MUSIC[1]))

def favorite_album(L: 'list of Albums') -> list:
    '''Takes in a list of Albums and returns the album
    with the most listening time'''
    L.sort(key = Album_listening_time, reverse = True)
    return L[0]
##print(favorite_album(MUSIC))

def top_n(L: 'list of Albums', n: int, keyfunction: 'function', t: bool) -> 'list of songdisplay':
    list_of_Songdisplays = all_Songdisplays(L)
    list_of_Songdisplays.sort(key=keyfunction, reverse=t)
    return list_of_Songdisplays[:n]
##print(top_n(MUSIC, 3, play_count_from_songdisplay, True))

def favorite_album2(L: 'list of Albums', keyfunction: 'function') -> list:
    L.sort(key = keyfunction, reverse = True)
    return L[0]
##print(favorite_album2(MUSIC, Album_listening_time))

def Collection_search(L: 'list of Album', s: str) -> 'list of Songdisplays':
    '''Takes in a collection and a string as parameters and returning a list of
    Songdisplays of songs whose title, artist, or album title include that string'''
    for a in L:
        x = Album_to_Songdisplays(a)
        for y in x:
            if s in y.a_title:
                return y
            elif s in y.s_title:
